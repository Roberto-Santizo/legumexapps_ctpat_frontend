...
export default function PackingListTable({ data, totals = [] }: PackingListProps) {
  // Calcular total de cajas sumando todos los productos
  const totalBoxes = totals.reduce((sum, item) => sum + item.total_boxes, 0);
    ...
    <table className="w-full border border-black text-[11px]">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td className="border border-black p-1">{totalBoxes || "-"}</td>
        </tr>
