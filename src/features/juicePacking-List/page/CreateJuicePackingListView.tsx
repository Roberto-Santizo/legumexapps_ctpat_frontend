...
import CreateJuicePackingListForm from "@/features/juicePacking-List/component/CreateJuicePackingListForm";
import { createJuicePackingListAPI } from "@/features/juicePacking-List/api/JuicePacking-ListAPI";
...
export default function CreateJuicePackingListView() {
  const navigate = useNavigate();

  const { id } = useParams();
  const ctpatId = Number(id);

  // Hook para actualizar el status del CTPAT
  const { mutate: updateStatus } = useUpdateCtpatStatus();

  const {register,handleSubmit,formState: { errors }} = useForm<CreateJuicePackingListFormData>();

  const { mutate: createPackingList } = useMutation({
    mutationFn: (data: CreateJuicePackingListFormData) =>
      createJuicePackingListAPI(ctpatId, data),

    onSuccess: async (res) => {
      toast.success(res.message);
      // Actualizar el status del CTPAT a 2 (siguiente paso: agregar items)
  ...
  const handleForm = (data: CreateJuicePackingListFormData) => {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-[var(--color-border-light)] overflow-hidden">
          ...
          <form
            ...
            noValidate
          >
            <CreateJuicePackingListForm register={register} errors={errors} />
